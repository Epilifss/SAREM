...
class VarejoModule:
    ...
    def __init__(self, user, parent):
        ...
        # Cabeçalho
        self.header = Header(self.root, self.user, caller_id="Varejo", tree=None)
        ...
        self.treeview = listagemTreeview(
            ...
            on_double_click=partial(exibir_detalhes_acompanhando, caller_id="Varejo", user=self.user)
        )
    ...
    def pesquisar_bo(self, event=None):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT bo_number, op, status, tipo_ocorrencia, setor_responsavel, loja FROM bo_records WHERE modulo LIKE 'varejo' AND status NOT LIKE 'Embarcado' AND bo_number LIKE ? AND D_E_L_E_T_ <> '*'", (f"%{termo}%",))
