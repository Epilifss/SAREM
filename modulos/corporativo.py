...
class CorporativoModule:
    ...
    def __init__(self, user, parent):
        ...
        # Cabeçalho
        self.header = Header(self.root, self.user, caller_id="Corporativo", tree=None)
        ...
        self.treeview = listagemTreeview(
            ...
            on_double_click=partial(exibir_detalhes_acompanhando, caller_id="Corporativo", user=self.user)
        )
    ...
    def pesquisar_bo(self, event=None):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT bo_number, op, status, tipo_ocorrencia, setor_responsavel, loja FROM bo_records WHERE modulo LIKE 'corporativo' AND status NOT LIKE 'Embarcado' AND bo_number LIKE ? AND D_E_L_E_T_ <> '*'", (f"%{termo}%",))
